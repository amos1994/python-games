...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            animation_index = self.age % self.lifespan
            canvas.draw_image(self.image, [self.image_center[0] + animation_index * self.image_size[0], self.image_center[1]],
...
def group_collide(group, sprite):
    ...
    for sp in group:
        if sp.collide(sprite):
            collisions += 1
            remove_list.add(sp)
            explosion_group.add(Sprite(sp.pos, [0,0], [0,0], [0,0], explosion_image, explosion_info, explosion_sound))
...
def group_group_collide(group1, group2):
    ...
    for sprite1 in group1:
        collisions = group_collide(group2, sprite1)
        score += collisions
...
def draw(canvas):
    ...
    #process missile group
    process_sprite_group(canvas, missile_group)

    #process explosion group
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        rock_vel = [score * random.random() * .6 - .3, score * random.random() * .6 - .3]
        rock_avel = random.random() * .2 - .1
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)

        # ensure enough distance from ship
        if dist(my_ship.pos, a_rock.pos) > 100:
